...
MemStore holds the data between `latest_snapshot_lsn` and `lates_lsn`. It consists of PageIndex that holds references to WAL records or pages, PageStore that stores recently materialized pages, and WalStore that stores recently received WAL.
...
We assume that PageIndex is few orders of magnitude smaller than addressed data hence it should fit memory. We also don't care about crash tolerance as we can rebuild it from snapshots metadata and WAL records from WalStore and Safekeeper.
...
I imagine that newly created pages would be just added to the back of PageStore (again in queue-like fashion) but this way there wouldn't be any meaningful ordering inside of that queue. But when we are forming a new incremental snapshot we may prohibit any updates to the current set of pages in PageStore (giving up on single page version rule) and cut off that whole set when snapshot creation is complete.
...
* how much time we will spend applying WAL to access certain pages with older LSN's?
* how many files do we create for a single database?

I can think of the following options here:

a) whole database goes to one full snapshot.
...
b) table segment is the unit of snapshotting
...
* for databases that are connected to us through the Safekeeper we can start loading a set of the latest snapshots or we can do that lazily based on getPage request (I'd better avoid doing that lazily for now without some access stats from the previous run).
...
1) Since our ToC/array of entries would be sorted by page number we can store the whole BufferTag only when realtion_id is changed and store only delta-encoded offsets for a given relation. That would reduce the average per-page metadata size to something less than 4 bytes instead of 26.
2) It makes sense to keep ToC at the beginning of the file to avoid extra seeks to locate it. Doesn't matter too much with the local file but matters quite a lot on S3 -- if we are accessing a lot of ~1Gb files with the size of metadata ~ 1Mb then the time to transfer this metadata would be comparable with access latency itself (which is about a half of a second). So by slurping metadata with one read of file header instead of N reads we can improve the speed of page server start by this N factor.
